...
changelog: PARTE 4/5 - IACT Específico con nombres de modelos corregidos
partes: 4/5
estado: completo
revision: v2.3.1 (nombres modelos en inglés)
---

# CLEAN CODE NAMING PRINCIPLES v2.3.1
...
## ⚠️ RECORDATORIO: CAMBIOS EN v2.3.1
...
### 25.1 Estado Actual (v2.3.1)

```
CAMBIO CRÍTICO v2.1.0 → v2.3.1:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

apps/core/ → ❌ DEPRECADO (ver ANALISIS_APP_CORE_v2.0.0.md)

Razones:
1. ServiceAccessService → NO queremos segmentación de datos
2. ETLService → ETL fuera de Django (stored procedures DB)
3. CallRecord → ETL crea tablas agregadas directamente
4. Confusión arquitectónica
...
├── utils/          ✅ Mixins compartidos (SoftDeleteMixin, etc.)
├── reports/        ✅ Consumo de datos (sin filtros por usuario)
├── dashboard/      ✅ Visualización con widgets (nueva app v2.3.1)
├── pipeline/       ✅ Monitoreo ETL
├── ivr/            ✅ Adaptador READ-ONLY (antes ivr_legacy)
...
# ════════════════════════════════════════════════════════════
# apps/utils/ - COMPARTIDOS
# ════════════════════════════════════════════════════════════

apps/utils/
├── models.py
│   ├── TimeStampedModel      # created_at, updated_at
│   └── SoftDeleteMixin       # is_deleted, deleted_at, etc.
│
├── mixins.py
│   └── SoftDeleteViewSetMixin  # destroy(), restore(), deleted()
│
├── pagination.py
│   ├── StandardPagination
│   └── LargePagination
...
# ════════════════════════════════════════════════════════════
# apps/dashboard/ - VISUALIZACIÓN CON WIDGETS (NUEVA v2.3.1)
# ════════════════════════════════════════════════════════════
...
├── ❌ CallRecord (ahora en apps/ivr/)
└── ❌ ServiceFilterMixin (NO filtrar por usuario)
```

---

### 25.3 Comparación Antes/Después

```python
# ════════════════════════════════════════════════════════════
# ANTES (v1.0 - CON apps/core/) ❌
# ════════════════════════════════════════════════════════════

# apps/core/services.py
class ServiceAccessService:
    """Segmentación de datos por Service 800."""  # ❌ NO queremos
    
    @staticmethod
    def filter_by_user_services(queryset, user):
        # Filtra datos según servicios asignados al usuario
        user_services = UserServiceAccess.objects.filter(user=user)
        return queryset.filter(servicio_800__in=user_services)
...
class ReportViewSet(viewsets.ModelViewSet):
    """
    Reportes con RBAC ÚNICAMENTE.
    
    - Todos los usuarios ven TODOS los datos
    - RBAC controla QUÉ pueden HACER con esos datos
    """  # ← Español
    ...
    def get_queryset(self):
        # ✅ SIN filtro por usuario - todos ven todo
    ...
    def export_excel(self, request):
        ...
        RBAC controla quién puede EXPORTAR,
        NO qué datos puede VER.
        """  # ← Español
        
        # Obtener TODOS los datos (sin filtro por usuario)
...
### 25.4 Flujo de Datos Actual

```
ARQUITECTURA IACT v2.3.1:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
### 26.5 DashboardService (Nueva App v2.3.1)
...
**Continúa en:** CLEAN_CODE_NAMING_PRINCIPLES_v2_3_1_PARTE_5.md
...
- ✅ 25. IACT: Separación access/ vs core/
  - apps/core/ DEPRECADO documentado
  - Nueva arquitectura sin segmentación
  - apps/dashboard/ agregada
- ✅ 26. IACT: Service Layer Pattern
  - ReportService completo
  - DashboardService (nueva app)
  - WidgetService
...
**Próxima parte:** Anti-patterns, Tabla Resumen, Referencias, Changelog v2.3.1
