...
class DashboardConfigFactory(DjangoModelFactory):
    """
    Factory para DashboardConfig (configuración de dashboard).
    
    Uso básico:
        config = DashboardConfigFactory(
            user=user,
...
class DefaultDashboardFactory(DashboardConfigFactory):
    """Factory para dashboard por defecto del usuario."""
    config_name = 'Default Dashboard'
    is_default = True


class PublicDashboardFactory(DashboardConfigFactory):
...
class WidgetConfigFactory(DjangoModelFactory):
    """
    Factory para WidgetConfig (configuración de widget).
    
    Uso básico:
        widget = WidgetConfigFactory(
            dashboard=dashboard,
    ...
    dashboard = factory.SubFactory(DashboardConfigFactory)
...
class CallsChartWidgetFactory(WidgetConfigFactory):
...
class TransfersChartWidgetFactory(WidgetConfigFactory):
...
class AbandonmentsChartWidgetFactory(WidgetConfigFactory):
...
class TopClientsWidgetFactory(WidgetConfigFactory):
...
class MetricsSummaryWidgetFactory(WidgetConfigFactory):
...
class QuarterlyFilterFactory(SavedFilterFactory):
...
class MonthlyFilterFactory(SavedFilterFactory):
...
class UserDashboardPreferenceFactory(DjangoModelFactory):
    """
    Factory para UserDashboardPreference (preferencias usuario).
    
    Uso básico:
        pref = UserDashboardPreferenceFactory(
            user=user,
    ...
    user = factory.SubFactory(UserFactory)
    default_dashboard = factory.SubFactory(DashboardConfigFactory)
...
class CompleteDashboardFactory:
    """
    Factory que crea dashboard completo con widgets.
    
    Uso:
        dashboard = CompleteDashboardFactory.create_dashboard(
            user=user,
            widgets=['calls', 'transfers', 'abandonments']
        )
    ...
    def create_dashboard(user, widget_types=None):
        ...
        config = DashboardConfigFactory(user=user)
...
class UserWithDashboardFactory(UserFactory):
    """
    ...
    def dashboard(self, create, extracted, **kwargs):
        ...
        config = DefaultDashboardFactory(user=self)
        
        # Crear 3 widgets estándar
        CallsChartWidgetFactory(dashboard=config, position_x=0, position_y=0)
        TransfersChartWidgetFactory(dashboard=config, position_x=1, position_y=0)
        AbandonmentsChartWidgetFactory(dashboard=config, position_x=2, position_y=0)
