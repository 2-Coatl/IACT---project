...
class QuarterlyReportFactory(ReportFactory):
...
class TransferReportFactory(ReportFactory):
...
class AbandonedReportFactory(ReportFactory):
...
class ClientReportFactory(ReportFactory):
...
class CustomReportFactory(ReportFactory):
...
class PendingExecutionFactory(ReportExecutionFactory):
...
class RunningExecutionFactory(ReportExecutionFactory):
...
class SuccessExecutionFactory(ReportExecutionFactory):
...
class FailedExecutionFactory(ReportExecutionFactory):
...
class QuarterlyTemplateFactory(ReportTemplateFactory):
...
class TransferTemplateFactory(ReportTemplateFactory):
...
class QuarterlyScheduleFactory(ReportScheduleFactory):
...
class WeeklyScheduleFactory(ReportScheduleFactory):
...
class ReportCacheFactory(DjangoModelFactory):
    """
    Factory para ReportCache (cache de reportes).
    
    Uso básico:
        cache = ReportCacheFactory(
...
class CompleteReportFactory:
    """
    Factory que crea reporte completo con ejecución.
    
    Uso:
        data = CompleteReportFactory.create_report(
            report_type='quarterly',
            user=user,
            success=True
        )
    ...
    def create_report(report_type, user, success=True):
        ...
        if success:
            execution = SuccessExecutionFactory(report=report)
        else:
            execution = FailedExecutionFactory(report=report)
...
class TemplateWithScheduleFactory:
    """
    Factory que crea template con schedule.
    
    Uso:
        data = TemplateWithScheduleFactory.create(
            report_type='quarterly',
            user=user
        )
